...
bool CSharpRpcGenerator::WriteClientToServerStub(const char* fileName)
{
	...
	for (map_iter; map_iter != g_rpcSchemaManager.clientToServerStructures.end(); map_iter++)
	{
		...
		fprintf(fp, "\t\protected abstract int DO_%s(TObject pTarget, Span<byte> pReqPacket);\n"
			, functionName.c_str());
	}
	fprintf(fp, "\t\protected abstract int DO_ERROR_PACKET(TObject pTarget, Span<byte> pReqPacket);\n");
	fprintf(fp, "\t\protected abstract int DO_ERROR_RESULT(TObject pTarget, int ret, int type);\n");
